...
def driver():
        for i in range(0,18):
                p[i] = p[i]^key[i%14]
                print("key",i+1,":",binary_value(hex(p[i])))
...
def encryption(data):
        ...
        for i in range(0,16):
                ...
                L1 = func(L)
                R = R^func(L1)
                print("Left : ", binary_value(hex(L)), "         Right : ", binary_value(hex(R)))
                L,R = swap(L,R)
        L,R = swap(L,R)
        L = L^p[17]
        R = R^p[16]
        encrypted = (L<<32) ^ R
        print("final ciphertext :",binary_value(hex(encrypted)))
        return encrypted
...
def decryption(data):
    ...
    print("final plaintext :",binary_value(hex(decrypted_data1)))
